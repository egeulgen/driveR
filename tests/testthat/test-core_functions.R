...
test_that("`predict_coding_impact` -- argument checks work", {
    ...
    nec_cols <- c("Gene.refGene", "SIFT_score", "Polyphen2_HDIV_score", "LRT_score",
        ...
        "DANN_score")
    expect_error(predict_coding_impact(path2annovar_csv), paste0("The table in `annovar_csv_path` should contain all of the following columns: ",
        paste(dQuote(nec_cols), collapse = ", ")))

    path2annovar_csv <- system.file("extdata/example.hg19_multianno.csv", package = "driveR")
    expect_error(predict_coding_impact(path2annovar_csv, keep_highest_score = "INVALID"),
        "`keep_highest_score` should be logical")

    expect_error(predict_coding_impact(path2annovar_csv, keep_single_symbol = "INVALID"),
...
test_that("`prioritize_driver_genes` -- argument check works", {
    ...
    expect_error(prioritize_driver_genes(example_features_table[, -1], "LUAD"), "`features_df` should contain exactly 27 columns")
    ...
    expect_error(prioritize_driver_genes(tmp, "LUAD"), paste0("`features_df` should contain the following columns: ",
