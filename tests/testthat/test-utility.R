...
test_that("`create_noncoding_impact_score_df` works", {
    ...
    expect_is(noncoding_df <- driveR:::create_noncoding_impact_score_df(path2annovar_csv),
        "data.frame")
...
test_that("`create_gene_level_scna_df` works", {
    ...
    expect_is(driveR:::create_gene_level_scna_df(example_scna_table, build = "GRCh38"),
        "data.frame")
...
test_that("`create_gene_level_scna_df` argument checks work", {
    ...
    expect_error(driveR:::create_gene_level_scna_df(tmp), paste0("`scna_df` should contain all of: ",
        paste(dQuote(nec_cols), collapse = ", ")))

    expect_error(driveR:::create_gene_level_scna_df(example_scna_table, gene_overlap_threshold = "INVALID"),
        "`gene_overlap_threshold` should be numberic")

    expect_error(driveR:::create_gene_level_scna_df(example_scna_table, gene_overlap_threshold = -1),
...
test_that("`create_SCNA_score_df` works", {
    ...
    expect_is(SCNA_scores_df2 <- driveR:::create_SCNA_score_df(gene_SCNA_df, build = "GRCh38"),
        "data.frame")
...
test_that("`create_SCNA_score_df` argument checks work", {
    ...
    expect_error(driveR:::create_SCNA_score_df(gene_SCNA_df, log2_ratio_threshold = "INVALID"),
        "`log2_ratio_threshold` should be numberic")


    expect_error(driveR:::create_SCNA_score_df(gene_SCNA_df, MCR_overlap_threshold = "INVALID"),
        "`MCR_overlap_threshold` should be numberic")

    expect_error(driveR:::create_SCNA_score_df(gene_SCNA_df, MCR_overlap_threshold = -1),
...
test_that("`determine_hotspot_genes` works", {
    path2annovar_csv <- system.file("extdata/example.hg19_multianno.csv", package = "driveR")
    expect_is(hotspot_genes <- driveR:::determine_hotspot_genes(path2annovar_csv),
        "character")
})

test_that("`determine_hotspot_genes` arg check works", {
    expect_error(driveR:::determine_hotspot_genes(path2annovar_csv, hotspot_threshold = "INVALID"),
...
test_that("`determine_double_hit_genes` works", {
    ...
    expect_is(dhit_genes <- driveR:::determine_double_hit_genes(path2annovar_csv,
        gene_SCNA_df), "character")

    path2annovar_csv <- system.file("extdata/example_cohort.hg19_multianno.csv",
        package = "driveR")
    expect_is(dhit_genes <- driveR:::determine_double_hit_genes(path2annovar_csv,
        gene_SCNA_df_batch, batch_analysis = TRUE), "character")
})

test_that("`determine_double_hit_genes` argument checks work", {
    path2annovar_csv <- system.file("extdata/example.hg19_multianno.csv", package = "driveR")
    expect_error(driveR:::determine_double_hit_genes(path2annovar_csv, gene_SCNA_df,
