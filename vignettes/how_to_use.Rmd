...
Finally, we can calculate the cancer driverness probability of each gene using `prioritize_driver_genes()`. For this function, `cancer_type` can be of the short names for the 21 different cancer types that was used to train the multi-task learning model utilized in this package:
...
# Example II - Batch Sequencing Data
...
As before, as input, `create_features_df()`, the function used to create the features table for driverness prediction, requires the following:
...
Finally, we can calculate the cancer driverness probability of each gene using `prioritize_driver_genes()`. 
